<?php  
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') 
    {
        ...
        $uploadDir = "../../resources/img/users/posts/boladas/";
        $deleted = "no";
        ...
        if($img_array != "")
        {
            if($insert_boladas_query = mysqli_query($con, "INSERT INTO `boladas`(`id`, `uname`, `name`, `img_array`, `description`, `price`, `location`, `status`, `deliver`, `date_added`, `clicks_num`, `deleted`) 
            VALUES (NULL,'$user','$item','$img_array','$description','$price','$location','$status','$deliver','$date_added','0','$deleted')"))
            {
                echo "success";
            }
            else
